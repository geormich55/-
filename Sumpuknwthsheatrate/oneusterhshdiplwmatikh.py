...
X1_ = pd.DataFrame(datas, columns=['LPStTemp', 'AmbTemp', 'CondFlow', 'InletCond', 'OutletCond', 'CondTemp', 'CondLevel', 'HeatRate'])
...
A1['CondLevel'] = A7
A1['CondLevel(t-1)'] = G
H = pd.DataFrame(datas, columns=['HeatRate'])
A8 = pd.DataFrame(datas, columns=['HeatRate'])
for i in range(2904):
...
for i in range(2905):
       n=0
       if Y1.values[i,0]<4000 and i<2903 and Y1.values[i,0]!=0:
           Y1.values[i, 0] = 0
           I.append(i)
           n=1
           Y1.values[i+1,0]=0
           ...
           Y1.values[i+2,0]=0
           I.append(i+2)
       elif Y1.values[i,0]>10000 and (i<2903) and Y1.values[i,0]!=0:
           Y1.values[i, 0] = 0
           I.append(i)
           n=1
           Y1.values[i+1,0]=0
