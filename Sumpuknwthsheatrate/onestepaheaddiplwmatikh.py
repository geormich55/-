...
X1 = pd.DataFrame(datas, columns=['LPStTemp', 'AmbTemp', 'CondFlow', 'InletCond', 'OutletCond', 'CondTemp', 'CondLevel', 'HeatRate'])
Y1 = pd.DataFrame(datas, columns=['HeatRate'])
Y2 = pd.DataFrame(datas, columns=['HeatRate'])
...
for i in range(2905):
    ...
    if Y1.values[i, 0] < 4000:
        mhdenika1 = mhdenika1 + 1
        m = 1

    elif Y1.values[i, 0] > 10000:
        mhdenika2 = mhdenika2 + 1
        m = 1
    ...
    if ((X2.values[i, 37] * X2.values[i, 38]) / 3600) != 0 and (X2.values[i, 45] / 3600) != 0:
        if abs((((X2.values[i, 44]) / ((X2.values[i, 37] * X2.values[i, 38]) / 3600)) - (1 / (X2.values[i, 45] / 3600)))) >= 0.05:
            mhdenika4=mhdenika4+1
            if m == 0:
                mhdenika3 = mhdenika3 + 1
mhdenikasunolo=mhdenika1+mhdenika2+mhdenika3
for i in range(2905):
       n=0
       if Y1.values[i,0]<4000 and i<2904 and Y1.values[i,0]!=0:
           Y1.values[i, 0] = 0
           I.append(i)
           n=1
           Y1.values[i+1,0]=0
           I.append(i+1)
       elif Y1.values[i,0]>10000 and (i<2904) and Y1.values[i,0]!=0:
           Y1.values[i, 0] = 0
           I.append(i)
           n=1
           Y1.values[i+1,0]=0
