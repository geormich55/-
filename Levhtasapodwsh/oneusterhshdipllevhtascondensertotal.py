...
#dataset = pd.read_excel(r'C:\Users\Micha\Desktop\Diplwmatikh ELPE\Σχέσεις\python kai MATLAB arxeia\Levhtasapodwsh\Dataslevhtas-condensertotal.xlsx', sheet_name='Fullo1')
dataset = pd.read_excel(r'Dataslevhtas-condensertotal.xlsx', sheet_name='Fullo1')
...
H38 = pd.DataFrame(datas, columns=['Condenserefficiency'])
A45 = pd.DataFrame(datas, columns=['Condenserefficiency'])
for i in range(2904):
           H38.values[i+1,0]=A45.values[i,0]
A1['Condenserefficiency'] = A45
A1['Condenserefficiency(t-1)'] = H38
X1=A1
Y1 = pd.DataFrame(datas, columns=['Condenserefficiency'])
Y2 = pd.DataFrame(datas, columns=['Condenserefficiency'])
for i in range(2904):
...
for i in range(2905):
       n=0
       if Y1.values[i,0]<0.3 and i<2903 and Y1.values[i,0]!=0:
           Y1.values[i, 0] = 0
           I.append(i)
           n=1
           Y1.values[i+1,0]=0
           ...
           Y1.values[i+2,0]=0
           I.append(i+2)
       elif Y1.values[i,0]>0.575 and (i<2903) and Y1.values[i,0]!=0:
           Y1.values[i, 0] = 0
           I.append(i)
           n=1
           Y1.values[i+1,0]=0
           ...
           Y1.values[i+2,0]=0
           I.append(i+2)
       if n == 0 and i < 2903 and Y1.values[i,0]!=0:
